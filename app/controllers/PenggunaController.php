...
class PenggunaController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				pengguna.id_user LIKE ? OR 
				pengguna.username LIKE ? OR 
				pengguna.password LIKE ? OR 
				pengguna.email LIKE ? OR 
				pengguna.foto LIKE ? OR 
				pengguna.account_status LIKE ?
			)";
			$search_params = array(
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("pengguna.id_user", ORDER_TYPE);
		}
		if($fieldname){
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("pengguna.id_user", $rec_id);; //select record based on primary key
		}
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("username","password","email","foto","account_status");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id_user","username","foto","account_status");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'username' => 'required',
			...
			if($this->validated()){
				$db->where("pengguna.id_user", $rec_id);;
	...
	function delete($rec_id = null){
		...
		$db->where("pengguna.id_user", $arr_rec_id, "in");
